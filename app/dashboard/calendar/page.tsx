...
export default function CalendarPage() {
  ...
  // Selected calendar day (defaults to today's date)
  ...
  const getDatePlusDaysStr = (days: number) => {
    const d = new Date();
    d.setDate(d.getDate() + days);
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  };

  // Sync modal reschedule state whenever a post is opened
  useEffect(() => {
    if (selectedPost) {
  ...
  // Select day on calendar cell click
  const handleCellClick = (day: number) => {
    if (isPastDay(day)) {
      showToast('Cannot select past dates. Please choose today or an upcoming date.', 'error');
      return;
    }
    setSelectedDay(day);
  };

  // Direct post creation for a specific day
  const handleDayCreatePost = (day: number) => {
    if (isPastDay(day)) {
    ...
    router.push(`/dashboard/create-post?scheduledAt=${year}-${monthStr}-${dayStr}T18:00`);
  };

  // Drop post on a day - PRESERVES exact original time
  const handleDropOnDay = async (postId: string, day: number) => {
    ...
    // Preserve exact original scheduled hour and minute if previously set
    let hour = 18;
    let minute = 0;
    if (post.scheduledAt) {
      ...
      const now = new Date();
      hour = isToday(day) ? Math.min(23, now.getHours() + 1) : 18;
      minute = 0;
    }

    const targetDate = new Date(year, month, day, hour, minute, 0, 0);

    // If dropped on today but the hour has already passed, adjust to next available hour
    if (isToday(day) && targetDate.getTime() < Date.now() - 60000) {
      ...
      if (nextHour < 24) {
        ...
        showToast('The scheduled time has already passed today. Please pick tomorrow or a later date.', 'error');
        return;
      }
    }
    ...
    // Select the dropped day to show its overview
    setSelectedDay(day);
    ...
    showToast(`Post scheduled for ${dayName} at ${formattedTime} (exact time kept)!`);
  ...
  // Quick schedule for today or tomorrow - PRESERVES exact original time
  const handleQuickSchedulePost = async (postId: string, targetDay: 'today' | 'tomorrow') => {
    ...
    if (post.scheduledAt) {
      ...
      const now = new Date();
      hour = targetDay === 'today' ? Math.min(23, now.getHours() + 1) : 18;
      minute = 0;
    }
    ...
    if (targetDay === 'today' && target.getTime() < Date.now() - 60000) {
      const nextHour = new Date().getHours() + 1;
      if (nextHour < 24) {
        hour = nextHour;
        minute = 0;
    ...
    showToast(`Post scheduled for ${targetDay === 'today' ? 'Today' : 'Tomorrow'} at ${formattedTime}!`);
  ...
  // Save reschedule changes from the Post modal
  const handleSaveModalReschedule = async () => {
    ...
    if (targetDate.getTime() < Date.now() - 60000) {
      showToast('Scheduled date and time cannot be in the past. Please select a future time.', 'error');
      return;
    }
    ...
    try {
      ...
      setPosts((prev) =>
        prev.map((p) =>
          p.id === selectedPost.id ? { ...p, scheduledAt: targetIso, status: 'SCHEDULED' } : p
        )
      );
      setSelectedPost((prev) =>
        prev && prev.id === selectedPost.id ? { ...prev, scheduledAt: targetIso, status: 'SCHEDULED' } : prev
      );
      ...
      showToast(`Post rescheduled to ${formattedDate} at ${formattedTime}!`);
    } catch (err) {
      console.error('Error rescheduling post:', err);
      showToast('Failed to update schedule. Please try again.', 'error');
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-7xl relative">
              ...
              <div
                ...
                className={`min-h-[120px] p-2 flex flex-col group transition-all relative ${
                  isPast
                    ...
                    : isSelected
                    ? 'bg-indigo-50/70 ring-2 ring-indigo-600 ring-inset shadow-xs z-10 cursor-pointer'
                    : isHovered
                    ? 'bg-indigo-100/70 ring-2 ring-indigo-500 ring-inset z-10 cursor-pointer'
                    : today
                    ? 'bg-indigo-50/30 ring-1 ring-indigo-200/80 cursor-pointer hover:bg-indigo-50/60'
                    : tomorrow
                    ? 'bg-amber-50/20 ring-1 ring-amber-200/70 cursor-pointer hover:bg-amber-50/50'
              ...
              >
                ...
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    {isSelected && !today && !tomorrow && (
                      <span className="text-[10px] font-bold text-indigo-700 bg-indigo-100/90 px-1.5 py-0.2 rounded-md">
                        Selected
                      </span>
                    )}
                    {isPast && (
      ...
      {/* SELECTED DAY ACTIONS & OVERVIEW BAR */}
      {selectedDay && !isPastDay(selectedDay) && (
        <div className="bg-gradient-to-r from-indigo-50/90 via-sky-50/70 to-indigo-50/90 p-4 rounded-2xl border border-indigo-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in fade-in">
          <div className="flex items-start md:items-center gap-3">
            <div className="h-11 w-11 rounded-xl bg-indigo-600 text-white flex flex-col items-center justify-center flex-shrink-0 shadow-xs">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs text-slate-500 font-medium">
                  · {getPostsForDay(selectedDay).length} {getPostsForDay(selectedDay).length === 1 ? 'post' : 'posts'} scheduled
                </span>
              </div>
              <p className="text-xs text-slate-600 mt-0.5">
                {getPostsForDay(selectedDay).length === 0
                  ? 'No posts scheduled on this date yet. You can drag any unscheduled draft here or click schedule.'
                  : 'Posts for this date are highlighted above. Click any post card to edit its schedule or view details.'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-end md:self-auto flex-shrink-0">
            <button
              onClick={() => handleDayCreatePost(selectedDay)}
            ...
            >
              <Plus className="w-3.5 h-3.5" /> Schedule Post for this Date
            </button>
          </div>
        </div>
      )}
      ...
      {selectedPost && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            {selectedPost.status !== 'PUBLISHED' && (
              <div className="p-4 rounded-2xl bg-indigo-50/70 border border-indigo-200/80 space-y-3.5">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-bold text-indigo-950 flex items-center gap-1.5">
                    <CalendarIcon className="w-4 h-4 text-indigo-600" /> Reschedule Date & Time
                  </p>
                  {selectedPost.scheduledAt && (
                    <span className="text-[11px] font-semibold text-indigo-700 bg-white px-2 py-0.5 rounded-md border border-indigo-200 shadow-2xs">
                      Current: {new Date(selectedPost.scheduledAt).toLocaleDateString()} at {new Date(selectedPost.scheduledAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                      <CalendarIcon className="w-3.5 h-3.5 text-indigo-600" /> Select Date:
                    </label>
                    <div className="flex items-center gap-1">
                      <button
                        type="button"
                        onClick={() => setRescheduleDate(getTodayDateStr())}
                        className={`px-2 py-1 rounded-lg text-[11px] font-bold border transition cursor-pointer ${
                          rescheduleDate === getTodayDateStr()
                      ...
                      <button
                        type="button"
                        onClick={() => setRescheduleDate(getTomorrowDateStr())}
                        className={`px-2 py-1 rounded-lg text-[11px] font-bold border transition cursor-pointer ${
                          rescheduleDate === getTomorrowDateStr()
                      ...
                      <button
                        type="button"
                        onClick={() => setRescheduleDate(getDatePlusDaysStr(7))}
                        className={`px-2 py-1 rounded-lg text-[11px] font-bold border transition cursor-pointer ${
                          rescheduleDate === getDatePlusDaysStr(7)
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-bold bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs cursor-pointer"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-indigo-600" /> Select Time:
                    </label>
                ...
                <button
                  ...
                  onClick={handleSaveModalReschedule}
                  className="w-full py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition shadow-xs flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  {isUpdatingSchedule ? (
                    <span>Saving Schedule...</span>
                  ) : (
                    <>
                      <Check className="w-4 h-4" /> Save Rescheduled Date & Time
                    </>
                  )}
                </button>
              </div>
